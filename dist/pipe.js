...
export async function runPipe(args, raw) {
    const format = !args.format || args.format === "auto"
        ? raw.trim().startsWith("<")
            ? "junit"
            : "json"
        : args.format;
    ...
    if (args.strict && (report.unmatched.length > 0 || report.failedOps > 0))
        return 1;
    return 0;
}
async function main() {
...
if (isMain()) {
    main().catch((err) => {
        console.error(`[kiwi-pipe] error: ${err.message}`);
        process.exit(1);
    });
}

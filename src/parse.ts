...
export function normalizeStatus(s: string | undefined): FrameworkStatus {
...
export function parseResultsJson(raw: string): TestResult[] {
  let data: unknown;
  try {
    data = JSON.parse(raw);
  } catch (err) {
    throw new Error(`Results are not valid JSON: ${(err as Error).message}`);
  }
  const obj = data as { tests?: unknown[]; results?: unknown[] };
